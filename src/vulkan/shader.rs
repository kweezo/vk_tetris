...
impl Shader{
    ...
    fn create_shader_module(device: &Device, vert_path: String, frag_path: String) -> (vk::ShaderModule, vk::ShaderModule){
        ...
        let vert_module = unsafe{device.get_ash_device().create_shader_module(&create_info, None)}.expect("Failed to create the vertex shader module");
        
        create_info.code_size = frag_src.len();
        create_info.p_code = frag_src.as_ptr() as *mut u32;
    

        let frag_module = unsafe{device.get_ash_device().create_shader_module(&create_info, None)}.expect("Failed to create the vertex shader module");
    ...
    pub fn new(device: &Device, vert_path: String, frag_path: String) -> Shader{
        ...
        Shader {vert_module: vert_module, frag_module: frag_module}
    }
