use ash::{vk::{self, MAX_EXTENSION_NAME_SIZE}, Entry};
use std::{ffi::CString, ffi::CStr, fs};
...
impl Instance{
    ...
    fn get_supported_extensions(entry: &Entry, extensions: &Vec<String>) -> Vec<CString>{
        let supported_extensions = unsafe{entry.enumerate_instance_extension_properties(None).expect("Failed to get enumerate instance extension properties")};
    ...
    fn get_supported_layers(entry: &Entry, layers: &Vec<String>) -> (Vec<CString>, Vec<String>){
        let available_layers = unsafe {entry.enumerate_instance_layer_properties().expect("Failed to enumreate supported instance layers")};
        ...
        for layer in layers{
            ...
            for supported_layer in &available_layers{
                ...
                if supported_layer.layer_name == *layer_v{
                    supported_layers.push(CString::new(layer.as_str()).expect("Failed to create a new CString"));
                    found = true;
                }
            }
    ...
    fn get_extensions(conf: &Config, entry: &Entry, window: &Window) -> (Vec<*const i8>, Vec<CString>){
        let mut glfw_extensions = window.get_glfw_context().get_required_instance_extensions().expect("Failed to get list of the required GLFW instance extensions");
        glfw_extensions.extend(conf.required_extensions.iter().cloned());

        let required_extensions_cstr: Vec<CString> = Instance::get_supported_extensions(entry, &glfw_extensions);
        ...
        let optional_extensions_cstr = Instance::get_supported_extensions(&entry, &conf.optional_extensions);
    ...
    pub fn new(entry: &Entry, window: &Window) -> Instance{
        ...
        let (supported_layers_cstr, unsupported_layers) = if conf.enable_layers {Instance::get_supported_layers(&entry, &conf.validation_layers)} else {(Vec::new(), Vec::new())};
        ...
        let (extensions_cstr_ptr, _extensions_cstr) = Instance::get_extensions(&conf, entry, window);

        let supported_layers_cstr_ptr: Vec<*const i8> = supported_layers_cstr
            .iter()
            .map(|s| s.as_ptr())
            .collect();
        ...
        let instance: ash::Instance = unsafe{entry.create_instance(&create_info, None).expect("Failed to create a vkInstance")};
