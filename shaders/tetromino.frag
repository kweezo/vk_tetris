...
void main() {
    ...
    for (uint i = 0; i < instance_count; i++) {
        vec2 pos = (instance.dat[i].pos);
        vec4 col = instance.dat[i].col;

        vec2 dist_vec = vec2(pos.x - gl_FragCoord.x, pos.y - gl_FragCoord.y);

        float dist = max(2 - sqrt(dist_vec.x * dist_vec.x + dist_vec.y * dist_vec.y), 0);
